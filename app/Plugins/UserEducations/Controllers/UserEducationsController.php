 ...
 * @since      2016-03-28
 */
namespace Plugins\UserEducations\Controllers;
...
class UserEducationsController extends Controller {
    ...
    public function index(Request $request) {
        ...
        if(!empty($educations->toArray())){
            return $this->response->item($educations, (new UserEducationTransformer));
        }else{
            return response()->json(['user_educations' => []]);
        }
    ...
    public function store(Request $request) {
        $user_education_data = $request->only('user_id', 'education', 'location', 'organization', 'certification_date', 'is_active');
        $user = $this->auth->user();
        $user_education_data['user_id'] = $user->id;
    ...
    public function edit($id) {
        ...
        return $this->response->item($user_education, (new UserEducationTransformer));
    }
    ...
    public function update(Request $request, $id) {
        $user_education_data = $request->only('education', 'location', 'organization', 'certification_date');
        $validator = Validator::make($user_education_data, UserEducation::GetValidationRule());
