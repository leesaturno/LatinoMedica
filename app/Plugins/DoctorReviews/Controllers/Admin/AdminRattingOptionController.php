...
namespace Plugins\DoctorReviews\Controllers\Admin;
...
use Plugins\DoctorReviews\Transformers\RattingOptionTransformer;
 ...
 */
class AdminRattingOptionController extends Controller {
    ...
    /**
     * Show all DoctorReviews
     *
     * Get a JSON representation of all the DoctorReviews.
     *
    ...
    public function index(Request $request) {
        $user_review = RattingOption::paginate(20);
        return $this->response->paginator($user_review, new RattingOptionTransformer());
    }
    ...
    public function store(Request $request) {
        $user_review_data = $request->only('option');
        $validator = Validator::make($user_review_data, RattingOption::GetValidationRule());
        if ($validator->passes()) {
            $user_review = RattingOption::create($user_review_data);
            if ($user_review) {
    ...
    public function edit($id) {
        $user_review = RattingOption::find($id);
        if (!$user_review) {
            return $this->response->errorNotFound("Invalid Request");
        }
        return $this->response->item($user_review, new RattingOptionTransformer);
    }
