...
use Plugins\DoctorReviews\Transformers\RatingTransformer;
 ...
 */
class AdminDoctorReviewsController extends Controller {
    ...
    public function index(Request $request) {
        $user_review = Rating::filterByRequest($request)->paginate(20);
        return $this->response->paginator($user_review, (new RatingTransformer)->setDefaultIncludes(['User','doctor_user']));
    }
    ...
    public function store(Request $request) {
        ...
        $validator = Validator::make($user_review_data, Rating::GetValidationRule());
        if ($validator->passes()) {
            $user_review_data['is_active'] = true;
            $user_review = Rating::create($user_review_data);
            if ($user_review) {
    ...
    public function edit($id) {
        $user_review = Rating::find($id);
        if (!$user_review) {
            return $this->response->errorNotFound("Invalid Request");
        }
        return $this->response->item($user_review, (new RatingTransformer)->setDefaultIncludes(['User']));
    }
