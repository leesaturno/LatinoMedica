...
class UsersController extends Controller
{
    ...
    public function index(Request $request) 
    {
        ...
        $goldMembership = User::with('user_profile')->where('users.subscription_id', config('constants.ConstMembershipPlan.Gold'))
                ...
                ->orderBy('users.points', 'desc')->skip($skip)->take(config('listing.per_page'))->get();
        $fractal = new Manager();
        ...
        if (count($goldMembership) < config('listing.per_page')) {
            ...
            $otherMembership = User::with('user_profile')->where('users.subscription_id', '!=', 0)
                    ->where('users.subscription_id', '!=', config('constants.ConstMembershipPlan.Gold'))
                    ->join('user_profiles', function ($join) {
                        $join->on('user_profiles.user_id', '=', 'users.id');
                    })
                    ->where('users.role_id', config('constants.ConstUserTypes.Doctor'))->where('is_active', 1)
            ...
            if (count($membership) < config('listing.per_page')) {
                ...
                $notVerifiedMembership = User::with('user_profile')->where('users.subscription_id', 0)
                        ->where('users.role_id', config('constants.ConstUserTypes.Doctor'))
                        ->join('user_profiles', function ($join) {
                            $join->on('user_profiles.user_id', '=', 'users.id');
                        })
    ...
    {
       if($username){
            $user = User::with('user_profile', 'attachments','UserEducations', 'workplace', 'badge', 'insured_patients')->where("username", $username)->first();
        }else{
