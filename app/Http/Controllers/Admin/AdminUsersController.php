...
class AdminUsersController extends Controller
{
    ...
    {
        $user_details = UserProfile::where('user_id','=',$id)->with(['User', 'Country', 'City'])->first()->toArray();
        if (!$user_details) {
            return $this->response->errorNotFound("Invalid Request");
        }
        $response = array();
        $response['id'] = $user_details['user']['id'];
        $response['role_id'] = $user_details['user']['role_id'];
        $response['email'] = $user_details['user']['email'];
        $response['username'] = $user_details['user']['username'];
        $response['first_name'] = $user_details['first_name'];
        $response['last_name'] = $user_details['last_name'];
        $response['gender_id'] = $user_details['gender_id'];
        if ($user_details['language_id'] != "") {
            $response['language_id'] = array_map('intval', explode(',', $user_details['language_id']));
        } else {
            $response['language_id'] = "";
        }
        $response['mobile'] = $user_details['user']['mobile'];
        $response['phone'] = $user_details['phone'];
        $response['address'] = $user_details['address'];
        $response['country_id'] = $user_details['country']['id'];
        $response['city_id'] = $user_details['city']['id'];
        $response['country_name'] = $user_details['country']['name'];
        $response['city_name'] = $user_details['city']['name'];
        $response['postal_code'] = $user_details['postal_code'];
        $response['latitude'] = $user_details['latitude'];
        $response['longitude'] = $user_details['longitude'];
        $response['awards'] = $user_details['awards'];
        $response['subscription_id'] = $user_details['user']['subscription_id'];
        $response['subscription_end'] = $user_details['user']['subscription_end'];
        $response['is_active'] = $user_details['user']['is_active'];
        $response['is_email_confirmed'] = $user_details['user']['is_email_confirmed'];
        $response['is_verified'] = $user_details['user']['is_verified'];
        $response['is_featured'] = $user_details['user']['is_featured'];
        $response['user_login_count'] = $user_details['user']['user_login_count'];
        $response['created_at'] = $user_details['user']['created_at'];
        $response['register_ip_id'] = $user_details['user']['register_ip_id'];
        $response['last_login_ip_id'] = $user_details['user']['last_login_ip_id'];
    ...
    {
        $user_data = $request->only('role_id','is_email_confirmed','awards','subscription_id','subscription_end','mobile','first_name','last_name','phone','postal_code','gender_id','country_id', 'city_id','country_name', 'city_name', 'state_id', 'address', 'subscription_id', 'awards', 'language_id', 'latitude', 'longitude', 'is_active', 'email','is_verified', 'is_featured');
		...
		$validRules = User::GetValidationRule();
		unset($validRules['email']);
		$validator = Validator::make($user_data,$validRules);
        $user = false;
        $userEmailCheck = $this->getEmailCheck($user_data['email'], $id);
        if ($userEmailCheck === true) {
             throw new \Dingo\Api\Exception\StoreResourceFailedException('User could not be updated. Email already exists.');
        }
        $subscribeFailMsg = "Subscription could not be updated. Please, try again.";
        ...
        if ($request->has('id')) {
            ...
            if ($request->subscription_id != $user->subscription_id) {
               $subscribeList = Subscribe::where('user_id', '=', $id)
                                          ->where('status', '!=', config('constants.ConstPaypalStatus.Cancel'))
                                          ->first();                         
               if (!isset($subscribeList->profile_id) || $subscribeList->subscribed_by == 1) {
                $storedValue = [
                    ...
                    $subscriptionDetails = Subscription::where('id', '=', $request->subscription_id)
                                            ->first()->toArray();
                    $subscription_end = date('Y-m-d', strtotime("+".$subscriptionDetails['period_days']." days"));
                   ...
                   $request['subscription_id'] = $user->subscription_id;
        ...
        if ($validator->passes() && $user && $subscriptionCheck === false) {
                ...
                $countryDetails = Country::where('name', '=', $request['country_name'])
                                                                ->first();
                if (isset($user_data['country_name']) && $user_data['country_name'] != "" && $countryDetails->id != $userProfile->country_id) {
                    if (!empty($countryDetails)) {
                        $user_data['country_id'] = $countryDetails->id;
                    }
                }
                if (isset($user_data['city_name']) && $user_data['city_name'] != "") {
                    $cityDetails = City::where('name', '=', $request['city_name'])
                                                                ->first();
                    if ($user_data['city_name'] != "" && ($cityDetails->id != $userProfile->city_id || empty($cityDetails))) {
                            $city_details = [
                                'name'       => $request['city_name'],
                                'country_id' => $user_data['country_id'],
                                'latitude'   => $request['latitude'],
               ...
               if (!empty($user_data['language_id'])) {
                    $languages = $user_data['language_id'];
                    $oldLanguages = $userProfile->language_id;
                    if ($oldLanguages != '') {
                        $oldLanguages = explode(',',$oldLanguages);
                        foreach($oldLanguages as $oldLanguage) {
                            $langOldDetail = Language::where('id', '=', $oldLanguage)->first();
                            /*$langcount = $langOldDetail['usage_count']-1;
                            if ($langcount < 0) {
                                $langcount = 0;
                            }
                            Language::where('id', '=', $oldLanguage)->update(['usage_count'=> $langcount]);*/
                         }
                    }
                    foreach($languages as $language) {
                      $langDetail = Language::where('id', '=', $language)->first();
                      //Language::where('id', '=', $language)->update(['usage_count'=> $langDetail['usage_count']+1]);
                    }
                    $user_profile_data['language_id'] = implode(',',$languages);
                }
                $user_profile_data['first_name']                 = $user_data['first_name'];
                $user_profile_data['last_name']                  = $user_data['last_name'];
                $user_profile_data['display_name']               = $user_data['first_name'].' '.$user_data['last_name'];
                ...
                $user_profile_data['latitude']                   = $user_data['latitude'];
                $user_profile_data['longitude']                  = $user_data['longitude'];
                $user_profile_data['insurance_id']               = 0;
                //$user_profile_data['is_online_booking_accepted'] = 1;
                $user_profile_data['awards']                     = $user_data['awards'];                
                UserProfile::where('user_id','=',$request->id)->update($user_profile_data);
        ...
        } else if ($subscriptionCheck === true) {
            throw new \Dingo\Api\Exception\StoreResourceFailedException($subscribeFailMsg);
        } else {
            throw new \Dingo\Api\Exception\StoreResourceFailedException('User could not be updated. Please, try again.', $validator->errors());
        }
    }
