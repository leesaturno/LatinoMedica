...
class AdminInsuranceCompaniesController extends Controller
{
    ...
    {
        $insurances = InsuranceCompany::filterByRequest($request)->paginate(20);
        return $this->response->paginator($insurances, (new InsuranceCompanyTransformer));
    }
    /**
    ...
    {
        $insurance_data = $request->only('name', 'is_active', 'insurance_type');
        $validator = Validator::make($insurance_data, InsuranceCompany::GetValidationRule());
        if ($validator->passes()) {
            $insurance_data['is_active'] = true;
            $insurance = InsuranceCompany::create($insurance_data);
            if ($insurance) {
    ...
    public function edit($id)
    {
        ...
        return $this->response->item($insurance, (new InsuranceCompanyTransformer));
    }
