    ...
    public function index(Request $request, $type = null) {
        ...
        if ($type == 'all' || $type == null) {
            ...
            if ($user->role_id == config('constants.ConstUserTypes.Doctor')) {
                $appointments = Appointment::where(['appointment_status_id' => config('constants.ConstAppointmentStatus.PendingApproval'),'doctor_user_id'=>$user->id])->orderBy('appointment_date', 'desc')->paginate(20);
            } else {
            ...
            if ($user->role_id == config('constants.ConstUserTypes.Doctor')) {
                $appointments = Appointment::where(['appointment_status_id' => config('constants.ConstAppointmentStatus.Approved'),'doctor_user_id'=>$user->id])->orderBy('appointment_date', 'desc')->paginate(20);
            } else {
            ...
            if ($user->role_id == config('constants.ConstUserTypes.Doctor')) {
                $appointments = Appointment::where(['appointment_status_id' => config('constants.ConstAppointmentStatus.Cancelled'),'doctor_user_id'=>$user->id])->orderBy('appointment_date', 'desc')->paginate(20);
            } else {
            ...
            if ($user->role_id == config('constants.ConstUserTypes.Doctor')) {
                $appointments = Appointment::where(['appointment_status_id' => config('constants.ConstAppointmentStatus.Rejected'),'doctor_user_id'=>$user->id])->orderBy('appointment_date', 'desc')->paginate(20);
            } else {
    ...
    public function booked(Request $request) {
        ...
        if (config('site.subscription_premium_doctor')) {
            ...
            $user_profiles = UserProfile::where('id', $storedValue['doctor_user_id'])
             ...
             $subscriptionDoctor = User::select(['subscription_id', 'id'])->where('id', '=', $storedValue['doctor_user_id'])->first()->toArray();
           if ($subscriptionDoctor['subscription_id'] < 1) {
