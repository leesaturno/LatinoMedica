...
class MessageTransformer extends Fractal\TransformerAbstract
{
    ...
    protected $availableIncludes = [
        'FromUser', 'ToUser', 'Messageable', 'Children', 'MessageContent', 'ItemUserStatus', 'DisputeStatus'
    ];
    ...
    {
        $output = array_only($message->toArray(), ['id', 'created_at', 'user_id', 'to_user_id', 'parent_id', 'item_id', 'message_content_id', 'message_folder_id', 'messageable_id', 'messageable_type', 'is_sender', 'is_starred', 'is_read', 'is_deleted', 'is_archived', 'is_review', 'is_communication', 'hash', 'size', 'dispute_status_id']);
     ...
     * @return Fractal\Resource\Item|null
     */
    public function includeChildren(Message $message)
    {
        if ($message->children) {
            return $this->collection($message->children, (new MessageTransformer())->setDefaultIncludes(['from_user', 'to_user', 'message_content', 'children']));           
        } else {
            return null;
        }
    }
