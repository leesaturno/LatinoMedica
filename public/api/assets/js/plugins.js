    ...
    module.controller('SubscriptionsController', function($scope, $state, $rootScope, $location, Flash, $filter, GENERAL_CONFIG, Planslist, GetSubscriptionDetail, PaymentGateway, SweetAlert, PaymentSuccessApprove, Subscribe, PlanAction) {
        ...
        $scope.planaction = function (actionType){
            if(actionType == 'active'){
                actParam = 'active';
                var titleMsg = 'Estás seguro de activar tu suscripción suspendida de PayPal?';
            }else if(actionType == 'suspend'){
                actParam = 'suspend';
                var titleMsg = 'Estás seguro de Suspender tu suscripción activada de paypal?';
            }else{
                actParam = 'cancel';
                var titleMsg = 'Está seguro de cancelar su suscripción a paypal?';
            }
             SweetAlert.swal({
                title: titleMsg,
                text: "",
        ...
        model.payment = function (){
            SweetAlert.swal({
                title: "Está seguro de suscribirse a este plan?",
                text: "",
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#DD6B55",
    ...
    module.controller('PagesController', function ($scope, $http, $filter, $state, $rootScope, $location, PageFactory, $translate, $translateLocalStorage, Cities, Specialties) {
        ...
        function footerLinks(){
            ...
            model.phaseOne = [
                ...
                {title:'Prensa',slug:'about-us'},
                {title:'Empleo',slug:'about-us'},
                {title:'Contacto',slug:'about-us'},
                {title:'Respuestas',slug:'about-us'},
                {title:'FAQ',slug:'faq'},
                {title:'Blog',slug:'blog'}
            ];
            model.phaseTwo = [
                {title:'Doctor',slug:'doctor'},
                {title:'Especialidad',slug:'speciality'},
                {title:'Procedimiento',slug:'procedure'},
                {title:'Ubicacion',slug:'location'},
                {title:'Seguro',slug:'insurance'}
            ];
            ...
            model.footerEnd = [
                {title:'Mapa del sitio',slug:'about-us'},
                {title:'Politica de Privacidad',slug:'about-us'},
                {title:'Política de uso de la web',slug:'about-us'},
                {title:'Diseño Web: Cincobe Publicidad & Marketing',slug:'about-us'}
            ];
        }
    });
}(angular.module("Abs.Pages")));
...
(function (module) {
    ...
    module.controller('PhotosController', function ($state, $scope, Flash, $filter, $rootScope, $location, Upload, GENERAL_CONFIG, PhotosFactory, PhotoDeleteFactory, SweetAlert, $window, $timeout) {
        ...
        $scope.photoDelete = function (id) {
            SweetAlert.swal({
                title: "Seguro que quieres eliminar esta foto de la clínica?",
                text: "",
                type: "warning",
                showCancelButton: true,
            ...
            function (isConfirm) {
                if (isConfirm) {
                    EducationDelete.delete({id: id}).$promise.then(function (response) {
                        Flash.set($filter("translate")("Se ha eliminado correctamente la Educación del usuario"), 'success', true);
                        $timeout(function(){
                            $state.reload();
                        },500);
                    }, function (error) {
                        Flash.set($filter("translate")("No se pudo eliminar la Educación del usuario"), 'error', false);
                    });
                }
            });
        };
...
(function (module) {
    ...
    module.controller('UserFavoriteController', function ($scope, $http, $filter, $state, $rootScope, $location, Flash, SweetAlert, MyDocotors, FavoriteAdd, FavoriteDelete) {
        ...
        $scope.remove_favorite = function (favoriteId) {
             SweetAlert.swal({
                title: "Está seguro de quitar a este médico en su favorito?",
                text: "",
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#DD6B55", confirmButtonText: "Yes",
